...
export const getAllTweets = () => async (dispatch) => {
    ...
    try {
        const { data } = await axios.get(`${API_BASE_URL}/api/twits/`, { headers: getAuthHeaders() });
        dispatch({ type: GET_ALL_TWEETS_SUCCESS, payload: data });
    } catch (error) {
        console.log("error", error);
        dispatch({ type: GET_ALL_TWEETS_FAILURE });
    }
};
...
export const createTweet = (tweetData) => async (dispatch) => {
    ...
    try {
        const { data } = await axios.post(`${API_BASE_URL}/api/twits/create`, tweetData, { headers: getAuthHeaders() });
        dispatch({ type: TWEET_CREATE_SUCCESS, payload: data });
    } catch (error) {
        console.log("error", error);
        dispatch({ type: TWEET_CREATE_FAILURE });
    }
};
